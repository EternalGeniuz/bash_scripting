...
#многострочный ввод приветственного баннера с правилами игры
cat <<'BANNER'
********************************************************************************
...
#генерация последовательности из 4х неповторяющихся цифр для загадки
gen_secret() {
  local s="" d used="::::::::::"
  while (( ${#s} < 4 )); do
    d=$((RANDOM % 10))
...
#функция для проверки корректности ввода, согласно правилам игры
is_valid_guess() {
  local g="$1"
...
#функция для подсчета "коров" и "быков"
count_cows_bulls() {
  local g="$1" s="$2"
...
#объявляю 3 массива для попаданий, коров и быков
...
#основной игровой цикл, читает ввод, выходит по q/Q, проверяет валидность ввода и подсчитывает результаты. 
#После каждого хода выводит историю ходов, а также проверяет угадал ли юзер число
while true; do
